...
class WongScraper(BaseScraper):
    """Scraper para Wong (wong.pe) — motor VTEX con URLs de departamento."""
    ...
    def scrape_category(self, page: Page, category: str, path: str) -> List[Product]:
        ...
        try:
            ...
            logger.warning(f"[Wong] Sin productos en {url}")
            return products

        items = page.query_selector_all(".vtex-product-summary-2-x-container")
        logger.debug(f"[Wong] {category}: {len(items)} elementos encontrados en DOM")
        ...
        for item in items:
            try:
                ...
                if not is_relevant(name, category):
                    logger.debug(f"[Wong] Filtrado: '{name}'")
                    continue
                ...
                logger.debug(f"[Wong] Error parseando item: {exc}")
